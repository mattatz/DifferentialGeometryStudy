...
impl Surface for Helicoid {
  ...
  fn point_at(&self, u: f64, v: f64) -> Point3f {
    ...
    self.plane.origin + self.plane.x_axis * u * v.cos() + self.plane.y_axis * u * v.sin() + self.plane.normal * self.c * v
  }
  ...
  fn dv_at(&self, u: f64, v: f64) -> Vector3f {
    let (u, v) = self.map(u, v);
    - self.plane.x_axis * u * v.sin() + self.plane.y_axis * u * v.cos() + self.plane.normal * self.c
  }
