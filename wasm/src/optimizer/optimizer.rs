...
pub trait Optimizer {
    fn new (x: &Matrix2x1f, delta: f64) -> Self where Self: Sized;
    ...
    fn backtrack (&self, expr: &F2x1, x: &Matrix2x1f, p: &Matrix2x1f, rho: f64, c1: f64) -> f64 {
        let x0 = expr(&x);
        let d = c1 * (grad(expr, &x, self.delta()).transpose() * p).get(0).unwrap();
        let mut alpha = 1.0;
        ...
        match next {
          Some(x) => {
            ...
            */
            ...
            self.b = 
              self.b - 
              (self.b * s * s_t * self.b) / (s_t * self.b * s).get(0).unwrap().clone() + 
              (y * y_t) / (y_t * s).get(0).unwrap().clone();
            self.x = x;
            Some(x)
          },
          None => {
